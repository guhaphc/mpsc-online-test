...
export default function StudyMaterialSubjectPage(){
 ...
 return <main className={"page "+(nightMode?"night-page":"")}><section className={"card study-shell "+(nightMode?"night-mode":"")} style={{"--note-scale":fontScale/100,"--note-font":fontFamily==="Serif"?"Noto Serif Devanagari, serif":fontFamily==="Sans"?"Plus Jakarta Sans, sans-serif":"Noto Sans Devanagari, sans-serif"} as React.CSSProperties}>
  ...
  <div className="study-tools"><div className="compact-tools" aria-label="Reading controls"><button type="button" className={"icon-btn "+(searchOpen?"active":"")} onClick={()=>setSearchOpen(v=>!v)} aria-label="Search notes">⌕</button><div className="font-step"><button type="button" className="icon-btn" onClick={()=>setFontScale(v=>Math.max(80,v-10))} aria-label="Decrease font size">A−</button><span className="font-size-label">{fontScale}%</span><button type="button" className="icon-btn" onClick={()=>setFontScale(v=>Math.min(140,v+10))} aria-label="Increase font size">A+</button></div><div className="font-menu"><button type="button" className={"icon-btn "+(fontOpen?"active":"")} onClick={()=>setFontOpen(v=>!v)} aria-label="Choose font">Aa</button>{fontOpen&&<div className="font-popover"><strong>Font</strong><button className={fontFamily==="Devanagari"?"chosen":""} onClick={()=>{setFontFamily("Devanagari");setFontOpen(false)}}>Aa Default</button><button className={fontFamily==="Serif"?"chosen":""} onClick={()=>{setFontFamily("Serif");setFontOpen(false)}}>Aa Serif</button><button className={fontFamily==="Sans"?"chosen":""} onClick={()=>{setFontFamily("Sans");setFontOpen(false)}}>Aa Sans</button></div>}</div><button type="button" className={"icon-btn "+(nightMode?"active":"")} onClick={()=>setNightMode(v=>!v)} aria-label="Toggle night reading mode">{nightMode?"☀️":"🌙"}</button><div className="language-switch" role="group" aria-label="Notes language"><button type="button" className={language==="Marathi"?"selected":""} onClick={()=>changeLanguage("Marathi")}>मराठी</button><button type="button" className={language==="English"?"selected":""} onClick={()=>changeLanguage("English")} disabled={translationLoading}>EN</button></div></div>{(searchOpen||query)&&<div className="search-box"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="नोट्समध्ये शोधा..." aria-label="Search notes" />{query&&<button type="button" onClick={()=>setQuery("")} aria-label="Clear search">×</button>}</div><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="नोट्समध्ये शोधा — उदा. भीमबेटका, मायक्रोलिथ्स, पुरापाषाण..." aria-label="Search notes" />{query&&<button type="button" onClick={()=>setQuery("")} aria-label="Clear search">×</button>}</div><div className="progress-card"><strong>{progress}%</strong><span>Completed</span><i><b style={{width:progress+"%"}}/></i></div></div>
...
.compact-tools{display:flex;align-items:center;justify-content:flex-end;gap:5px;flex:1}.icon-btn{height:34px;min-width:34px;padding:0 7px;border:1px solid #d9e0e8;border-radius:9px;background:#fff;color:#334155;font:800 12px "Plus Jakarta Sans",sans-serif;cursor:pointer}.icon-btn:hover,.icon-btn.active{background:#e9f4f2;color:#0f766e;border-color:#b9ddd8}.font-step{display:flex;align-items:center;gap:2px;border:1px solid #d9e0e8;border-radius:10px;padding:2px;background:#f8fafc}.font-size-label{min-width:35px;text-align:center;font:700 9px "Plus Jakarta Sans",sans-serif;color:#64748b}.font-menu{position:relative}.font-popover{position:absolute;z-index:50;right:0;top:40px;width:145px;padding:8px;background:#fff;border:1px solid #d9e0e8;border-radius:12px;box-shadow:0 12px 30px rgba(20,35,50,.16)}.font-popover strong{display:block;padding:4px 8px 7px;font:800 10px "Plus Jakarta Sans",sans-serif;color:#64748b;text-transform:uppercase}.font-popover button{display:block;width:100%;border:0;background:transparent;text-align:left;padding:8px;border-radius:7px;color:#334155;font:600 12px "Plus Jakarta Sans",sans-serif;cursor:pointer}.font-popover button:hover,.font-popover button.chosen{background:#e9f4f2;color:#0f766e}.night-toggle{flex:0 0 auto;border:1px solid #d9e0e8;background:#fff;color:#334155;border-radius:12px;padding:9px 12px;font:700 12px "Plus Jakarta Sans",sans-serif;cursor:pointer;transition:.2s}.night-toggle.on{background:#172033;color:#f8fafc;border-color:#344054}.language-switch{display:flex;flex:0 0 auto;border:1px solid #d9e0e8;border-radius:12px;padding:3px;background:#f8fafc}.language-switch button{border:0;background:transparent;border-radius:9px;padding:9px 12px;font:700 12px "Noto Sans Devanagari",sans-serif;color:#64748b;cursor:pointer}.language-switch button.selected{background:#0f766e;color:#fff;box-shadow:0 2px 7px rgba(15,118,110,.16)}.language-switch button:disabled{opacity:.6;cursor:wait}.night-mode{background:#111827!important;border-color:#273449!important;color:#e5e7eb}.night-mode .study-tools{background:#182231;border-color:#273449}.night-mode .study-layout aside{background:#141d2a;border-color:#273449}.night-mode .study-layout article{background:#111827}.night-mode .toc{color:#c2ccda}.night-mode .toc:hover{background:#1e2b3b;color:#8de3d5}.night-mode .toc.active{background:#183b3a;color:#a8f0e5;border-color:#28635e}.night-mode .toc-title{color:#7f8da3}.night-mode .note-head{background:linear-gradient(145deg,#182635,#142b2a);border-color:#2b4b4a;box-shadow:none}.night-mode .note-head h2{color:#f1f5f9}.night-mode .note-head p{color:#aab7c7}.night-mode .note-para{color:#d7dee8}.night-mode .source-chip{background:#2a2519;color:#e7c979;border-color:#544625}.night-mode .fact{background:#1b1d20;border-color:#35404c;color:#e5e7eb;box-shadow:none}.night-mode .fact span{color:#cbd5e1}.night-mode .fact strong{color:#e5b957}.night-mode .search-result{background:#18302f;border-color:#28534f;color:#bde8e1}.night-mode .note-actions{border-color:#273449}.night-mode .note-actions .secondary{background:#182231;color:#dbe4ef;border-color:#3a4656}.night-mode .progress-card{background:#182231;border-color:#334155}.night-mode .progress-card span{color:#94a3b8}.night-mode .progress-card strong{color:#7ee0d2}.night-mode .source-note{background:#141c28;border-color:#273449;color:#8e9bab}.night-mode .compact-tools .icon-btn{background:#182231;color:#e5e7eb;border-color:#3a4656}.night-mode .compact-tools .icon-btn:hover,.night-mode .compact-tools .icon-btn.active{background:#183b3a;color:#9ce9df;border-color:#28635e}.night-mode .font-step{background:#111827;border-color:#3a4656}.night-mode .font-size-label{color:#aab7c7}.night-mode .font-popover{background:#182231;border-color:#3a4656}.night-mode .font-popover strong{color:#94a3b8}.night-mode .font-popover button{color:#dbe4ef}.night-mode .font-popover button:hover,.night-mode .font-popover button.chosen{background:#183b3a;color:#9ce9df}.night-mode .study-tools input{background:#111827;border-color:#3a4656;color:#e5e7eb}.night-mode .study-tools input::placeholder{color:#7f8da3}.night-mode .study-tools input:focus{border-color:#3f9d93;box-shadow:0 0 0 4px rgba(63,157,147,.12)}.night-mode .language-switch{background:#111827;border-color:#3a4656}.night-mode .language-switch button{color:#aab7c7}.night-mode .language-switch button.selected{background:#176b63;color:#fff}.search-box{position:relative;flex:1;display:flex;align-items:center}.search-box>span{position:absolute;left:15px;font-size:23px;color:#64748b;line-height:1;pointer-events:none}.search-box input{width:100%;padding-right:42px!important}.search-box button{position:absolute;right:8px;border:0;background:transparent;color:#64748b;font-size:22px;cursor:pointer;padding:5px 8px}.study-tools input{flex:1;border:1px solid #d9e0e8;border-radius:13px;padding:14px 16px;font:500 14px 'Noto Sans Devanagari',sans-serif;background:#fbfcfe;color:#172033;outline:none;transition:.2s}
...
.note-head h2{font-family:var(--note-font);font-size:clamp(23px,4vw,33px);line-height:1.25;margin:8px 0 5px;color:#172033;letter-spacing:-.4px}.note-head p{font-family:var(--note-font);margin:0;color:#657386;font-size:calc(13px * var(--note-scale))}
...
.note-para mark,.fact mark{background:#ffe58a;color:#172033;padding:1px 3px;border-radius:4px;box-shadow:0 0 0 1px rgba(180,130,0,.12)}.note-para{font-family:var(--note-font);font-size:calc(16px * var(--note-scale));line-height:2;color:#303d50;margin:18px 5px;max-width:850px;letter-spacing:.05px}
