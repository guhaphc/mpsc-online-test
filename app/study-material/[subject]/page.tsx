...
export default function StudyMaterialSubjectPage(){
 ...
 return <main className={"page "+(nightMode?"night-page":"")}><section className={"card study-shell "+(nightMode?"night-mode":"")} style={{"--note-scale":fontScale/100,"--note-font":fontFamily==="Serif"?"Noto Serif Devanagari, serif":fontFamily==="Sans"?"Plus Jakarta Sans, sans-serif":"Noto Sans Devanagari, sans-serif"} as React.CSSProperties}>
  ...
  <div className="study-tools"><div className="reading-controls" aria-label="Reading controls"><button type="button" className="font-btn" onClick={()=>setFontScale(v=>Math.max(80,v-10))} aria-label="Decrease font size">−</button><span className="font-size-label">{fontScale}%</span><button type="button" className="font-btn" onClick={()=>setFontScale(v=>Math.min(140,v+10))} aria-label="Increase font size">+</button><select className="font-select" value={fontFamily} onChange={e=>setFontFamily(e.target.value as "Devanagari"|"Serif"|"Sans")} aria-label="Change reading font"><option value="Devanagari">Default</option><option value="Serif">Serif</option><option value="Sans">Sans</option></select></div><button type="button" className={"night-toggle "+(nightMode?"on":"")} onClick={()=>setNightMode(v=>!v)} aria-label="Toggle night reading mode">{nightMode?"☀️ Day":"🌙 Night"}</button><div className="language-switch" role="group" aria-label="Notes language"><button type="button" className={language==="Marathi"?"selected":""} onClick={()=>changeLanguage("Marathi")}>मराठी</button><button type="button" className={language==="English"?"selected":""} onClick={()=>changeLanguage("English")} disabled={translationLoading}>English</button></div><div className="search-box"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="नोट्समध्ये शोधा — उदा. भीमबेटका, मायक्रोलिथ्स, पुरापाषाण..." aria-label="Search notes" />{query&&<button type="button" onClick={()=>setQuery("")} aria-label="Clear search">×</button>}</div><div className="progress-card"><strong>{progress}%</strong><span>Completed</span><i><b style={{width:progress+"%"}}/></i></div></div>
...
.reading-controls{display:flex;align-items:center;gap:3px;flex:0 0 auto;border:1px solid #d9e0e8;border-radius:12px;padding:3px;background:#f8fafc}.font-btn{width:32px;height:32px;border:0;border-radius:8px;background:transparent;color:#334155;font:800 20px "Plus Jakarta Sans",sans-serif;cursor:pointer}.font-btn:hover{background:#e8eef3}.font-size-label{min-width:38px;text-align:center;font:700 10px "Plus Jakarta Sans",sans-serif;color:#64748b}.font-select{height:32px;border:0;border-left:1px solid #d9e0e8;margin-left:3px;padding:0 6px;background:transparent;color:#334155;font:700 11px "Plus Jakarta Sans",sans-serif;outline:none}.night-toggle{flex:0 0 auto;border:1px solid #d9e0e8;background:#fff;color:#334155;border-radius:12px;padding:9px 12px;font:700 12px "Plus Jakarta Sans",sans-serif;cursor:pointer;transition:.2s}.night-toggle.on{background:#172033;color:#f8fafc;border-color:#344054}.language-switch{display:flex;flex:0 0 auto;border:1px solid #d9e0e8;border-radius:12px;padding:3px;background:#f8fafc}.language-switch button{border:0;background:transparent;border-radius:9px;padding:9px 12px;font:700 12px "Noto Sans Devanagari",sans-serif;color:#64748b;cursor:pointer}.language-switch button.selected{background:#0f766e;color:#fff;box-shadow:0 2px 7px rgba(15,118,110,.16)}.language-switch button:disabled{opacity:.6;cursor:wait}.night-mode{background:#111827!important;border-color:#273449!important;color:#e5e7eb}.night-mode .study-tools{background:#182231;border-color:#273449}.night-mode .study-layout aside{background:#141d2a;border-color:#273449}.night-mode .study-layout article{background:#111827}.night-mode .toc{color:#c2ccda}.night-mode .toc:hover{background:#1e2b3b;color:#8de3d5}.night-mode .toc.active{background:#183b3a;color:#a8f0e5;border-color:#28635e}.night-mode .toc-title{color:#7f8da3}.night-mode .note-head{background:linear-gradient(145deg,#182635,#142b2a);border-color:#2b4b4a;box-shadow:none}.night-mode .note-head h2{color:#f1f5f9}.night-mode .note-head p{color:#aab7c7}.night-mode .note-para{color:#d7dee8}.night-mode .source-chip{background:#2a2519;color:#e7c979;border-color:#544625}.night-mode .fact{background:#1b1d20;border-color:#35404c;color:#e5e7eb;box-shadow:none}.night-mode .fact span{color:#cbd5e1}.night-mode .fact strong{color:#e5b957}.night-mode .search-result{background:#18302f;border-color:#28534f;color:#bde8e1}.night-mode .note-actions{border-color:#273449}.night-mode .note-actions .secondary{background:#182231;color:#dbe4ef;border-color:#3a4656}.night-mode .progress-card{background:#182231;border-color:#334155}.night-mode .progress-card span{color:#94a3b8}.night-mode .progress-card strong{color:#7ee0d2}.night-mode .source-note{background:#141c28;border-color:#273449;color:#8e9bab}.night-mode .reading-controls{background:#111827;border-color:#3a4656}.night-mode .font-btn{color:#e5e7eb}.night-mode .font-btn:hover{background:#263447}.night-mode .font-size-label{color:#aab7c7}.night-mode .font-select{background:#111827;color:#e5e7eb;border-color:#3a4656}.night-mode .study-tools input{background:#111827;border-color:#3a4656;color:#e5e7eb}.night-mode .study-tools input::placeholder{color:#7f8da3}.night-mode .study-tools input:focus{border-color:#3f9d93;box-shadow:0 0 0 4px rgba(63,157,147,.12)}.night-mode .language-switch{background:#111827;border-color:#3a4656}.night-mode .language-switch button{color:#aab7c7}.night-mode .language-switch button.selected{background:#176b63;color:#fff}.search-box{position:relative;flex:1;display:flex;align-items:center}.search-box>span{position:absolute;left:15px;font-size:23px;color:#64748b;line-height:1;pointer-events:none}.search-box input{width:100%;padding-right:42px!important}.search-box button{position:absolute;right:8px;border:0;background:transparent;color:#64748b;font-size:22px;cursor:pointer;padding:5px 8px}.study-tools input{flex:1;border:1px solid #d9e0e8;border-radius:13px;padding:14px 16px;font:500 14px 'Noto Sans Devanagari',sans-serif;background:#fbfcfe;color:#172033;outline:none;transition:.2s}
