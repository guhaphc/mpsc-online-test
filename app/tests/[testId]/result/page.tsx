  ...
  useEffect(() => {
    (async () => {
      ...
      if (
        ...
        setError("This result link is invalid.");
        setLoading(false);
        return;
      }
      ...
      const [
        ...
        supabase
          .from("mpsc_attempts")
          .select(
            "id, test_id, user_id, score, submitted_at"
          )
          .eq("id", attemptId)
          .eq("test_id", id)
          .eq("user_id", user.id)
      ...
      if (firstError || !attemptResult.data) {
        setError(
          firstError?.message ||
            "The result could not be found."
        );
        setLoading(false);
        return;
      }
      ...
      const {
        ...
        .from("mpsc_answers")
        .select(
          "question_id, selected_answer, is_correct, marks"
        )
      ...
      setAttempt(attemptResult.data as Attempt);
      setTest(testResult.data as TestRecord);
      setQuestions(
    ...
    <main className="page">
      <section className="card result-card">
        ...
        <div className="result-grid">
          ...
          <div>
            <strong className="incorrect-value">
              {incorrect}
            </strong>
            <span>Incorrect</span>
          </div>
